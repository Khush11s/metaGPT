...
PREFIX_TEMPLATE = """You are a {profile}, named {name}, your goal is {goal} and preffered coding language should be {code_language}, and the constraint is {constraints}. """
...
Do not answer anything else, and do not add any other information in your answer. And Your Coding language should be {code_language}.
"""
...
{history}
{name}: {result}
And Your Coding language should be {code_language}.
"""
...
class Role:
    ...
    def __init__(self, name="", profile="", goal="", constraints="", desc="",code_language=""):
        self._llm = LLM()
        self._setting = RoleSetting(name=name, profile=profile, goal=goal, constraints=constraints, desc=desc,code_language=code_language)
        self._states = []
        self._actions = []
    ...
    async def _think(self) -> None:
        ...
        if len(self._actions) == 1:
            ...
            self._set_state(0)
            return
        logger.info("llm prompt template starts here ")
        logger.info(STATE_TEMPLATE)
        logger.info("llm prompt template ends here")
        
        prompt = self._get_prefix()
        logger.info("llm prompt starts here")
        logger.info(prompt)
        logger.info("llm prompt ends here")
        prompt += STATE_TEMPLATE.format(history=self._rc.history, states="\n".join(self._states),
                                        n_states=len(self._states) - 1)
        logger.info("llm formated prompt starts here")
        logger.info(prompt)
        logger.info("llm formated prompt ends here")
        next_state = await self._llm.aask(prompt)
        logger.info("llm response start")
        logger.info(next_state)
        logger.info("llm response ends")
