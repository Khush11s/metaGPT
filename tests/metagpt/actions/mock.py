...
The game will be developed as a console application in , which will allow it to be platform-independent. The game logic will be implemented using Object Oriented Programming principles. 
...
's in-built data structures like lists and dictionaries will be used extensively to manage the game state, player inventory, room details, etc. 

For testing, we can use the PyTest framework. This is a mature full-featured  testing tool that helps you write better programs.
...
PROJECT_MANAGEMENT_SAMPLE = '''## Required  third-party packages: Provided in requirements.txt format
```
"pytest==6.2.5"
```
...
## Logic Analysis: Provided as a  list[str, str]. the first is filename, the second is class/method/function should be implemented in this file. Analyze the dependencies between the files, which work should be done first
```
[
    ("object.py", "Object"),
    ("room.py", "Room"),
...
## Task list: Provided as  list[str]. Each str is a filename, the more at the beginning, the more it is a prerequisite dependency, should be done first
```
[
    "object.py", 
    "room.py", 
...
```
"""
Shared knowledge for this project includes understanding the basic principles of Object Oriented Programming, 's built-in data structures like lists and dictionaries, and the PyTest framework for testing. 
"""
```
...
MEILI_ERROR = '''/usr/local/bin/3.9 /Users/alexanderwu/git/metagpt/examples/search/meilisearch_index.py
